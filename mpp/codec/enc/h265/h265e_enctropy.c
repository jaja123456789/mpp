...
void h265e_cabac_writeOut(H265eCabacCtx_t *cabac_ctx)
{
...
#if 0
    if (cabcac_ctx->m_bIsCounter) {
        cabcac_ctx->m_fracBits += sbacGetEntropyBits(mstate, binValue);
        cabcac_ctx->m_fracBits = (cabcac_ctx->m_fracBits/* + (1<<CABAC_EST_Accuracy_CutDown-1)*/) & (0xffffffff << CABAC_EST_Accuracy_CutDown);
        return;
    }
#endif
...
void h265e_cabac_encodeBinTrm(H265eCabacCtx_t *cabac_ctx, RK_U32 binValue)
{
    ...
    if (m_bIsCounter) {
        ...
        m_fracBits = (m_fracBits /*+ (1<<CABAC_EST_Accuracy_CutDown-1)*/) & (0xffffffff << CABAC_EST_Accuracy_CutDown);
        return;
    }
#endif
    h265e_dbg_func("enter\n");
...
void h265e_cabac_finish(H265eCabacCtx_t *cabac_ctx)
{
    ...
    if (cabcac_ctx->m_bIsCounter) {
        ...
        h265e_stream_write_with_log(s, 0, RK_U32(cabcac_ctx->m_fracBits >> 15));
        cabcac_ctx->m_fracBits &= 32767;
        assert(0);
    }
#endif
    h265e_dbg_func("enter\n");

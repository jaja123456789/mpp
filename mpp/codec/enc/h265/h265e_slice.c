...
RK_S32 getNumRpsCurrTempList(H265eReferencePictureSet* rps)
{
...
void h265e_code_slice_header(H265eSlice *slice, MppWriteCtx *bitIf)
{
    ...
    if (!slice->m_dependentSliceSegmentFlag) {
        ...
        if (slice->m_pps->m_listsModificationPresentFlag && getNumRpsCurrTempList(rps) > 1) {
            ...
            if (refPicListModification->m_refPicListModificationFlagL0) {
                RK_S32 numRpsCurrTempList0 = getNumRpsCurrTempList(rps);
...
void code_skip_flag(H265eSlice *slice, RK_U32 abs_part_idx, DataCu_t *cu)
{
    // get context function is here
    H265eCabacCtx_t *cabac_ctx = &slice->m_cabac;
...
void code_merge_index(H265eSlice *slice, RK_U32 abs_part_idx)
{
    (void)abs_part_idx;
    H265eCabacCtx_t *cabac_ctx = &slice->m_cabac;
...
void code_split_flag(H265eSlice *slice, RK_U32 abs_part_idx, RK_U32 depth, DataCu_t *cu)
{
    H265eSps *sps = slice->m_sps;
    H265eCabacCtx_t *cabac_ctx = &slice->m_cabac;
    ...
    h265e_cabac_encodeBin(cabac_ctx, &slice->m_contextModels[OFF_SPLIT_FLAG_CTX + ctx], currSplitFlag);
}


void encode_cu(H265eSlice *slice, RK_U32 abs_part_idx, RK_U32 depth, DataCu_t *cu)
{
    H265eSps *sps = slice->m_sps;
    RK_U32 bBoundary = 0;
    ...
    code_merge_index(slice, abs_part_idx);
    return;
}


void proc_cu8(H265eSlice *slice, DataCu_t *cu, RK_U32 pos_x, RK_U32 pos_y, RK_S32 partIdx)
{
    (void)partIdx;
    (void)slice;
    RK_S32 nSize = 8;
...
void proc_cu16(H265eSlice *slice, DataCu_t *cu, RK_U32 pos_x, RK_U32 pos_y, RK_S32 partIdx)
{
    (void)partIdx;
    RK_U32  m;
    H265eSps *sps = slice->m_sps;
    ...
    for (m = 0; m < 4; m ++) {
        ...
        proc_cu8(slice, cu, cu_x_1, cu_y_1, m);
    }
}


void proc_cu32(H265eSlice *slice, DataCu_t *cu, RK_U32 pos_x, RK_U32 pos_y, RK_S32 partIdx)
{
    (void)partIdx;
    RK_U32 m;
    H265eSps *sps = slice->m_sps;
    ...
    for (m = 0; m < 4; m ++) {
        cu_x_1 = pos_x + (m & 1) * (nSize >> 1);
        cu_y_1 = pos_y + (m >> 1) * (nSize >> 1);
        proc_cu16(slice, cu, cu_x_1, cu_y_1, m);
    }

}

void proc_ctu(H265eSlice *slice, DataCu_t *cu)
{
    H265eSps *sps = slice->m_sps;
    RK_U32 k, m;
    ...
    for (m = 0; m < 4; m ++) {
        ...
        proc_cu32(slice, cu, cu_x_1, cu_y_1, m);
    }
